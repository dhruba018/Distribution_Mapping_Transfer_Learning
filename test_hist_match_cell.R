...
run <- function(seed = NULL, K = 100, density = FALSE, verbose = TRUE) {
...
prediction <- DMTL(target_set = list("X" = X1, "y" = Y1), source_set = list("X" = X2, "y" = Y2),
                   use_density = density, sample_size = 1e3, random_seed = seed, all_pred = TRUE)
...
# results_gdsc <- run(seed = 4, density = FALSE, K = 200, verbose = TRUE)


summarize_results <- function(results) {

cc1 <- cbind(results[, c("DMTL.CC1", "DMTL_SS.CC1", "BL.CC1")]);   cc1 <- rbind(cc1, mean = colMeans(cc1))
colnames(cc1) <- c("DMTL", "DMTL_SS", "BL");    #print(cc1)

cc2 <- cbind(results[, c("DMTL.CC2", "DMTL_SS.CC2", "BL.CC2")]);   cc2 <- rbind(cc2, mean = colMeans(cc2))
colnames(cc2) <- c("DMTL", "DMTL_SS", "BL");    #print(cc2)

nrmse1 <- cbind(results[, c("DMTL.NRMSE1", "DMTL_SS.NRMSE1", "BL.NRMSE1")])
nrmse1 <- rbind(nrmse1, mean = colMeans(nrmse1));     colnames(nrmse1) <- c("DMTL", "DMTL_SS", "BL");    #print(nrmse1)

nmae1 <- cbind(results[, c("DMTL.NMAE1", "DMTL_SS.NMAE1", "BL.NMAE1")])
nmae1 <- rbind(nmae1, mean = colMeans(nmae1));        colnames(nmae1) <- c("DMTL", "DMTL_SS", "BL");    #print(nmae1)

res_sum <- rbind(PCC = cc2["mean", ], SCC = cc1["mean", ], NRMSE = nrmse1["mean", ], NMAE = nmae1["mean", ]);    print(res_sum)
res_sum

}


results_gdsc <- run(seed = 4, density = FALSE, K = 150, verbose = FALSE)
summary_gdsc <- summarize_results(results_gdsc)
